...
const CommentForm = React.memo(({ id }: TCommentForm): JSX.Element => {
  ...
  const ratings = Object.entries(RATING).reverse().map(([key, value]) => <Rating key={key} title={key} value={value} checked={form.rating.toString() >= value.toString()} onChange={updateRatingValue} />);
  ...
  return (
    <form onSubmit={submitReview} className="reviews__form form" action="#" method="post" >
      ...
      <textarea className="reviews__textarea form__textarea" id="review" name="review" placeholder="Tell how was your stay, what you like and what can be improved" onChange={updateCommentValue} value={form.comment}></textarea>
